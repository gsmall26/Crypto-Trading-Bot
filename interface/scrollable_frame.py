...
class ScrollableFrame(tk.Frame):
    def __init__(self, *args, **kwargs):
        ...
        self.canvas.create_window((0,0), window=self.sub_frame, anchor="nw") #puts sub_frame in canvas

        self.canvas.configure(yscrollcommand=self.vsb.set) #links scrollbar and canvas
    ...
    def _on_mousewheel(self, event: tk.Event):
        self.canvas.yview_scroll(int(-1 * (event.delta / 60)), "units") #increase sensitivity of scrollbar by reducing the denominator (60)
