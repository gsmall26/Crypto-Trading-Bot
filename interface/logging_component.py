...
class Logging(tk.Frame):
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.logging_text = tk.Text(self, height=10, width=60, state=tk.DISABLED, bg=BG_COLOR, 
                                    fg=FG_COLOR_2, font=GLOBAL_FONT, highlightthickness=False, bd=0) #self designates the frame, height = # of lines, state=locked or not; have to unlock when writing to widget
        self.logging_text.pack(side=tk.TOP) #places widget in the frame

    #add new log
    def add_log(self, message: str):
        self.logging_text.configure(state=tk.NORMAL) #unlocks tk.Text widgets
